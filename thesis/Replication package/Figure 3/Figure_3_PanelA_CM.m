...
% volatility and skewness for market without circuit breaker
...
for n = 2:trigger
    ...
    S_CM_MC(n) = (1-weight(n)) * PRICE(V_CM(Z_sim_ind_l(n),n), VB_CM(Z_sim_ind_l(n),n), lambda, eta_MC(n))...
                 +weight(n) * PRICE(V_CM(Z_sim_ind_u(n),n), VB_CM(Z_sim_ind_u(n),n), lambda, eta_MC(n));
end
...
for m = 1:(N_min-1)
    S_CM_min(m) = min(S_CM_MC(60*(m-1)+1: min(60*m+1,trigger)));
end
...
for n = 1:length(PDiff_set)
   ...
   if ~isempty(I_min)
      ...
      if I_min(i) >10 % exclude the first minute
         ...
         Ret_MC = log(S_CM_MC(60*(I_min(i)-1)+2:60*I_min(i)+1)) - log(S_CM_MC(60*(I_min(i)-1)+1:60*I_min(i))); 
